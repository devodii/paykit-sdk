...
export class LemonSqueezyProvider
  ...
  private _throwIfFailed<T>(
    result: { ok: boolean; value?: LemonSqueezyResponse<T>; error?: unknown },
    operation: string,
  ): T {
    if (!result.ok || !result.value?.data) {
      throw new OperationFailedError(operation, this.providerName, {
        cause: new Error(
          result.error ? JSON.stringify(result.error) : 'Unknown LemonSqueezy error'
        ),
      });
    }
    ...
    const payload: any = {
      data: {
        ...
        attributes: {
          ...
          checkout_data: {
            email: params.customer && 'email' in params.customer ? params.customer.email : undefined,
            custom: params.metadata,
          },
        },
        relationships: {
          store: {
            data: { type: 'stores', id: String(params.provider_metadata?.store_id) },
          },
          variant: {
            data: { type: 'variants', id: String(params.provider_metadata?.variant_id) },
          },
        },
      },
    };

    const response = await this._client.post<LemonSqueezyResponse<LemonSqueezyCheckout>>(
      '/checkouts',
  ...
  async retrieveCheckout(id: string): Promise<Checkout | null> {
    const response = await this._client.get<LemonSqueezyResponse<LemonSqueezyCheckout>>(
      `/checkouts/${id}`,
    );
  ...
  async updateCheckout(id: string, params: UpdateCheckoutSchema): Promise<Checkout> {
    throw new Error('updateCheckout not natively supported by LemonSqueezy API');
  }
  ...
  async createCustomer(
    ...
    const response = await this._client.post<LemonSqueezyResponse<LemonSqueezyCustomer>>(
      '/customers',
      { body: JSON.stringify(payload) },
    );
  ...
  async retrieveCustomer(id: string): Promise<Customer | null> {
    const response = await this._client.get<LemonSqueezyResponse<LemonSqueezyCustomer>>(
      `/customers/${id}`,
    );
    if (!response.ok) return null;
  ...
  async updateCustomer(id: string, params: UpdateCustomerParams): Promise<Customer> {
    ...
    const response = await this._client.patch<LemonSqueezyResponse<LemonSqueezyCustomer>>(
      `/customers/${id}`,
      { body: JSON.stringify(payload) },
    );
  ...
  async retrieveSubscription(id: string): Promise<Subscription | null> {
    const response = await this._client.get<LemonSqueezyResponse<LemonSqueezySubscription>>(
      `/subscriptions/${id}`,
    );
    if (!response.ok) return null;
  ...
  async updateSubscription(
    ...
    const response = await this._client.patch<LemonSqueezyResponse<LemonSqueezySubscription>>(
      `/subscriptions/${id}`,
      { body: JSON.stringify(payload) },
    );
  ...
  async retrievePayment(id: string): Promise<Payment | null> {
    const response = await this._client.get<LemonSqueezyResponse<LemonSqueezyOrder>>(
      `/orders/${id}`,
    );
    if (!response.ok) return null;
  ...
  async createRefund(params: CreateRefundSchema): Promise<Refund> {
    ...
    const response = await this._client.post<LemonSqueezyResponse<LemonSqueezyOrder>>(
  ...
  async handleWebhook(
    ...
    if (!webhookSecret) {
      throw new ConfigurationError('Webhook secret is required for LemonSqueezy');
    }

    const signature = payload.headersAsObject['x-signature'];
    if (!signature) {
      throw new OperationFailedError('handleWebhook', this.providerName, {
        cause: new Error('Missing x-signature header'),
      });
    }

    const hmac = crypto.createHmac('sha256', webhookSecret);
    const digest = Buffer.from(hmac.update(payload.body).digest('hex'), 'utf8');
