...
function SdkLogo({ size = 32, priority = false }: { size?: number; priority?: boolean }) {
  return (
    <span
      ...
      aria-hidden="true"
    >
      <Image src="/payment-light.png" alt="" fill className="object-contain rounded-[6px] dark:hidden" priority={priority} />
      <Image src="/payment-dark.png" alt="" fill className="object-contain rounded-[6px] hidden dark:block" priority={priority} />
    </span>
  )
}
...
const MOBILE_LINKS = [
  ...
  { label: "GitHub", href: "https://github.com/payrouteshq/paykit-sdk", external: true },
]
...
export function Navbar() {
  ...
  return (
    <>
      <header className="fixed top-0 inset-x-0 z-50 h-14 bg-background/90 backdrop-blur-md border-b border-border/50">
        <div className="mx-auto flex h-full w-full max-w-6xl items-center gap-3 px-4 sm:gap-6 sm:px-5">
          ...
          <div className="flex min-w-0 shrink-0 items-center gap-2 sm:gap-2.5">
            ...
            >
              <Payroutes className="h-6 w-6 text-foreground" aria-hidden="true" />
            </Link>
            <span className="text-muted-foreground/40 text-base font-light select-none">/</span>
            <Link href="/" className="flex min-w-0 items-center gap-2 sm:gap-2.5" aria-label="Payment SDK home">
              <SdkLogo size={30} priority />
              <span className="hidden truncate text-[14px] font-semibold tracking-tight text-foreground leading-none sm:inline">
                Payment{" "}
          ...
          <div className="hidden min-w-0 flex-1 items-center gap-0.5 md:flex">
            ...
            <Link href="/changelog" className="h-8 px-3 flex items-center rounded-md text-[13px] text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors">
              Changelog
            </Link>
          </div>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-1">
            <Link
              ...
              rel="noopener noreferrer"
              aria-label="GitHub"
              className="hidden md:flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
            >
              <GitHubMark />
            </Link>
            ...
            <Link
              ...
              className="hidden md:inline-flex h-8 px-3.5 items-center rounded-md bg-foreground text-background text-[13px] font-medium hover:opacity-80 transition-opacity ml-1"
            >
              Get started
            </Link>
            ...
            <button
              ...
              className="flex md:hidden h-8 w-8 items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
            >
            ...
            <div className="px-4 py-4">
              <div className="space-y-0.5">
                {MOBILE_LINKS.map((item) => (
                  <Link
                    ...
                    onClick={() => setMobileOpen(false)}
                    className="flex items-center rounded-xl px-2 py-2.5 text-[13px] text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
                  >
                    {item.label}
                  </Link>
                ))}
              ...
              <div className="mt-4">
                <Link
                  href="/docs/getting-started"
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center justify-center h-9 rounded-xl bg-foreground text-background text-[13px] font-medium"
                >
                  Get started
                </Link>
              </div>
