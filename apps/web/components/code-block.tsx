...
type Language = SuggestedString<
  'tsx' | 'typescript' | 'bash' | 'json' | 'shell' | 'sh' | 'zsh'
>;
...
export function CodeBlock({
  ...
  if (!mounted)
    return (
      <div
        className={cn(
          'bg-muted h-24 w-full animate-pulse rounded-xl',
          className,
        )}
      />
    );
  ...
  const onCopy = () =>
    handleCopy({ text: children, message: 'Copied to clipboard' });
        ...
        {highlightLines?.length ? (
          <div
            style={{
              ...
              minWidth: '100%',
              width: 'max-content',
              background: isDarkTheme ? '#0a0a0a' : '#fafafa',
            }}
          >
            {children
              .trim()
              ...
              .map((line, i) => {
                ...
                if (!line)
                  return <div key={i} style={{ height: '1.4em' }} />;
                return (
                  <div
                    key={i}
                    className={cn(
                      '[&_*]:!bg-transparent [&_pre]:!m-0 [&_pre]:!p-0',
                      hl &&
                        'border-primary/50 bg-primary/5 -mx-5 border-l-2 pr-5 pl-[18px]',
                    )}
                  >
                    <SyntaxHighlighter
      ...
      className={cn(
        ...
        isFloating &&
          (isDark
            ...
            : 'bg-muted/80 border backdrop-blur-sm'),
      )}
      onClick={onClick}
    >
      {copied ? (
