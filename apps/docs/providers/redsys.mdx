...
Redsys has no public API client to hand back, so `_native` throws. Other providers return either their official SDK or [our HTTP client](/concepts/http-client).
