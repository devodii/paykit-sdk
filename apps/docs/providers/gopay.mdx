...
const webhook = paykit.webhooks
  ...
  .on('invoice.generated', async event => {
    /* emitted alongside payment.created on PAID */
  })
...
| `PAID`                   | `payment.created` + `invoice.generated` (+ `subscription.created` if recurring) |
| `AUTHORIZED`             | `payment.updated`                                                  |
| `CANCELED`               | `payment.canceled` (+ `subscription.canceled` if recurrence STOPPED) |
| `TIMEOUTED`              | `payment.canceled`                                                 |
