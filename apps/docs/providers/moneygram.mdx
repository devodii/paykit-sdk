...
`paykit._native` returns PayKit's [HTTP client](/concepts/http-client), already pointed at MoneyGram and authenticated with your credentials. Use it to call endpoints PayKit does not map yet.

```typescript
const result = await paykit._native.get(
  '/status/v1/transactions/tx_123',
);
